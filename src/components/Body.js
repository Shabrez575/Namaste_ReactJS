...
const Body = () => {
  ...
  const fetchData = async () => {
    try {
      const swiggyURL = 'https://www.swiggy.com/dapi/restaurants/list/v5?lat=25.5940947&lng=85.1375645&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING';
      ...
      const actualJSON = JSON.parse(proxyData.contents);

      const restaurants = actualJSON?.data?.cards[4]?.card?.card?.gridElements?.infoWithStyle?.restaurants || [];

      setListOfRest(restaurants);
      setFilteredRest(restaurants);
    } catch (error) {
      console.error("Error fetching Swiggy data through proxy:", error);
    }
  };
  ...
  if (onlineStatus === false) {
    return (
      <h1 className="onlineStatus m-[250px] flex justify-center">
        Looks Like you are offline! Please check your internet connection. <span>💔</span>
      </h1>
    );
  }
      ...
      <div className="filter flex justify-between mx-15">
        <div className="search">
          <input
            ...
            className="search-box border border-solid border-black rounded-lg m-4 px-2"
            value={searchText}
            onChange={(e) => setSearchText(e.target.value)}
          />
          <button
            className="search-btn px-4 py-1 m-3 bg-orange-400 border border-dark rounded-sm text-md hover:bg-orange-600 text-white cursor-pointer"
            onClick={() => {
              const filteredData = listOfRest.filter((res) =>
                res.info.name.toLowerCase().includes(searchText.toLowerCase())
              );
        ...
        <button
          className="px-4 py-1 m-3 bg-orange-400 border border-dark rounded-sm text-md hover:bg-orange-600 text-white cursor-pointer"
          onClick={() => {
